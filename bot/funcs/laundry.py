...
async def laundry_start_timer_func(
    repo: Repository, user_id: int, callback_data: str
) -> tuple[int, datetime]:
    """
    ...
    minutes = getattr(
        settings, callback_data.replace(UserCallback.START_LAUNDRY_PREFIX, "")
    )
    start_time = datetime_now()
    end_time = start_time + timedelta(minutes=minutes)

    await repo.laundry.save_or_update_laundry(
