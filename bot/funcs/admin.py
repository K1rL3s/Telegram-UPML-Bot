...
async def load_one_lesson_func(
    ...
    await repo.lessons.save_or_update_lessons(lessons_id, lessons_date, grade)
    for image_id, letter in zip(class_ids, "АБВ"):
        await repo.lessons.save_or_update_lessons(image_id, lessons_date, grade, letter)
...
async def get_meal_by_date(
    ...
    """
    menu = await repo.menu.get_menu_by_date(menu_date)
...
async def get_users_for_notify(
    ...
    return await repo.user.get_users_by_conditions(conditions)
