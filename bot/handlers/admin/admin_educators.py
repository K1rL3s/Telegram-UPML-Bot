...
async def edit_educators_date_handler(
    ...
    Обработчик ввода доты для изменения расписания воспитателей.
    """
    edit_date = date_by_format(message.text)

    if not edit_date:  # is False
        ...
        text = f'<b>Дата</b>: <code>{format_date(edit_date)}</code>\n' \
               f'<b>Расписание</b>:\n{schedule}\n\n' \
...
async def edit_educators_confirm_handler(
    ...
    await repo.save_or_update_educators_schedule(
        edit_date, new_schedule, callback.from_user.id
    )

    text = f'*Расписание воспитателей* на ' \
           f'*{format_date(edit_date)}* успешно изменено!'
